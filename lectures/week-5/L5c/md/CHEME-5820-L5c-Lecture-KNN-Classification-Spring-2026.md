...
> * __Explain the KNN classification algorithm:__ Describe how KNN computes distances to a query point, selects the $K$ nearest neighbors, and assigns the majority label. Explain how the vote is computed using the indicator function and why the choice of $K$ affects the decision boundary.
> * __Connect distance metrics to kernel functions:__ Explain that distances and kernels are both similarity measures, but they rank neighbors in opposite directions (small distance vs large kernel value). Describe how kernelized KNN replaces the distance metric with a kernel similarity to enable nonlinear decision boundaries.
> * __Apply kernelized KNN to nonlinear problems:__ Use the kernelized KNN procedure to classify points by ranking neighbors with a kernel value rather than a distance. Explain when unweighted vs kernel-weighted voting is used and how kernel choice affects the neighborhood.
