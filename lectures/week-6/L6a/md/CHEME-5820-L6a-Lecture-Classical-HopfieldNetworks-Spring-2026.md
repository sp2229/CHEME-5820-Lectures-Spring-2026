...
>
> [▶ Advanced numerical verification: convergence and recall-vs-load](CHEME-5820-L6a-Advanced-Classical-HopfieldConvergence-NumericalVerification-Spring-2026.ipynb). This notebook runs Julia simulations that verify monotonic energy descent along retrieval trajectories. It also quantifies how recall success changes with load $\alpha=K/N$ to connect theory to computation.

___

## Origin story: McCulloch-Pitts Neurons
In [their paper, McCulloch and Pitts (1943)](https://link.springer.com/article/10.1007/BF02478259) explored how the brain could produce highly complex patterns by using many [interconnected _basic cells (neurons)_](https://en.wikipedia.org/wiki/Biological_neuron_model). McCulloch and Pitts suggested a _highly simplified model_ of a neuron that has become the foundation for artificial neural networks that we find in wide use today. 
...
\end{align*}
$$
where $\sigma:\mathbb{R}\rightarrow\mathbb{R}$ is an _activation function_ that maps the weighted sum of a vector of inputs to a scalar (binary) output, $\boldsymbol{\theta}_{k} = (w_{k1}, w_{k2}, \ldots, w_{km}, -b_k)$ is a _vector of parameters (weights and bias)_ for neuron $k$, and $\hat{\mathbf{n}}(t) = (n_1(t), n_2(t), \ldots, n_m(t), 1)$ is the input vector augmented with a bias term.
...
Before we encode memories, fix the notation: each image has size $n\times n$ pixels, and we assume one neuron per pixel. Therefore, the network size is $N=n^2$.

> __Indexing convention used below:__
>
> We use memory indices $\mu\in\{1,\dots,K\}$ for stored patterns and neuron indices $i,j\in\{1,\dots,N\}$ for components of a state vector. Thus, stored memories are written as $\boldsymbol{\xi}^{\mu}\in\{-1,1\}^{N}$, while the current network state is $\mathbf{s}\in\{-1,1\}^{N}$ with components $s_i$.
>
> We use the standard Hopfield normalization by network size, i.e., the Hebbian sum is scaled by $1/N$, not $1/K$.

Suppose we wish our network to memorize $K$ images. We encode each image using the rule: if a pixel is white, set its memory value to `1`; if a pixel is black, set its memory value to `-1`. Define Hebbian weights componentwise as:
$$
\begin{align*}
w_{ij} &= \frac{1}{N}\sum_{\mu=1}^{K}\xi_i^{\mu}\xi_j^{\mu},\quad i\neq j,\\
w_{ii} &= 0,\quad i=1,\ldots,N.
\end{align*}
$$
where $\boldsymbol{\xi}^{\mu}$ denotes the $\mu$-th stored memory pattern. Equivalently, form the outer-product sum $\frac{1}{N}\sum_{\mu=1}^{K}\boldsymbol{\xi}^{\mu}(\boldsymbol{\xi}^{\mu})^{\top}$, then set diagonal entries to zero to enforce no self-connections.
The resulting weight matrix is $\mathbf{W}=[w_{ij}]\in\mathbb{R}^{N\times N}$: one row and one column per neuron, so each entry $w_{ij}$ is the coupling strength between neurons $i$ and $j$.
...
<div>
  <center>
    <img
      src="figs/Fig-Hopfield-Convergence-Landscape-Schematic.svg"
      alt="Hopfield convergence landscape schematic"
      height="360"
      width="980"
    />
  </center>
...
\end{equation*}
$$
where $w_{ij}\in\mathbf{W}$ are the weights of the network, $s_{i}\in\{-1,1\}$ are the states of neuron $i$, and $b_{i}$ is a bias term (typically set to zero but can be used to control activation thresholds).

Define the local field at neuron $i$ as:
$$
\begin{equation*}
h_i(\mathbf{s}) = \sum_{j=1}^{N} w_{ij}s_j + b_i.
\end{equation*}
$$
How do we go from a random state vector (memory) $\hat{\mathbf{s}}$ to a local minimum of the energy function $E(\mathbf{s})$? 
...
2. **Asynchronous update**: Choose a random neuron (node) $i$ and compute its local field $h_i(\mathbf{s}) = \sum_{j}w_{ij}s_{j}+b_{i}$. Then update using $s_{i}^{\prime} \leftarrow \texttt{sign}\left(h_i(\mathbf{s})\right)$, where $\texttt{sign}(\cdot)$ is the sign (activation) function and ties are handled by keeping the current state when $h_i(\mathbf{s})=0$.
...
5. **Check for convergence**: There are several criteria we can use to stop the iteration and determine if the network has converged:
   ...
   - _Memory retrieval_: Alternatively, if the current state $\mathbf{s}$ exactly matches any stored memory pattern $\boldsymbol{\xi}^{\mu}$ (Hamming distance = 0), then set $\texttt{converged}\gets\texttt{true}$, and return $\mathbf{s}$ as the retrieved memory.
   - _Fixed-point check_: If every neuron already matches the tie-preserving update rule $s_i=\texttt{sign}(h_i(\mathbf{s}))$ (or remains unchanged when $h_i(\mathbf{s})=0$), then set $\texttt{converged}\gets\texttt{true}$ and return $\mathbf{s}$.
...
> __Example:__
> 
> [▶ Analyze a classical Hopfield Network](CHEME-5820-L6a-Advanced-Classical-HopfieldConvergence-HebbianLearning-Proof-Spring-2026.ipynb). In this example, we analyze an example of a classical Hopfield Network to understand how it encodes and retrieves binary patterns using Hebbian learning and asynchronous updates. We consider uncorrelated binary patterns and investigate the network's ability to recover original patterns from noisy inputs.

___

### Convergence and Retrieval Guarantees
The pseudo code above is guaranteed to converge. However, let's separate convergence into two questions: why asynchronous updates stop, and what kind of state they stop at.

> __Convergence Theorem (Classical Hopfield, asynchronous updates):__
>
> Let $\mathbf{s}\in\{-1,+1\}^{N}$, assume $w_{ij}=w_{ji}$ and $w_{ii}=0$ (weight matrix is symmetric and has zero diagonal, i.e., no self-connections), and define the local field $h_i(\mathbf{s})$ and overall network energy $E(\mathbf{s})$ as:
> $$
> \begin{align*}
> h_i(\mathbf{s}) &= \sum_{j=1}^{N} w_{ij}s_j + b_i\\
...
> \end{align*}
> $$
> Here, $h_i(\mathbf{s})$ is the local field at neuron $i$, and $E(\mathbf{s})$ is the network energy.
...
>
> If one asynchronous update changes only neuron $i$ using a tie-preserving sign rule (if $h_i(\mathbf{s})=0$, keep $s_i$ unchanged; and $s_j'=s_j$ for $j\neq i$), then the one-step energy change is given by:
> $$
> \Delta E = E(\mathbf{s}')-E(\mathbf{s}) = -(s_i' - s_i)h_i(\mathbf{s})\leq 0.
> $$
> If $s_i'\neq s_i$, then $\Delta E<0$. Because the state space has size $2^N$ and the energy is lower-bounded, repeated asynchronous updates reach a fixed point in finitely many accepted flips. For the full derivation, see [Why Classical Hopfield Networks Converge and Why Hebbian Learning Works](CHEME-5820-L6a-Advanced-Classical-HopfieldConvergence-HebbianLearning-Proof-Spring-2026.ipynb).

The theorem guarantees that updates stop, but it does not guarantee retrieval of the intended memory. A complementary theorem characterizes when a stored pattern is itself a stable endpoint under Hebbian weights.

> __Retrieval Stability Theorem (Hebbian signal-crosstalk form):__
>
> Let $\{\boldsymbol{\xi}^{\mu}\}_{\mu=1}^{K}$ denote the stored-memory set and let $\mathbf{s}^{(0)}$ denote the retrieval cue (typically a noisy or incomplete input). During retrieval, we only observe the evolving state $\mathbf{s}(t)$; to test whether a specific stored pattern is a stable fixed point, choose index $\nu$ and evaluate at the candidate state $\mathbf{s}=\boldsymbol{\xi}^{\nu}$.
>
> Under Hebbian weights $w_{ij}=\frac{1}{N}\sum_{\mu=1}^{K}\xi_i^{\mu}\xi_j^{\mu}$ for $i\neq j$ and $w_{ii}=0$, the local field decomposes as:
> $$
> h_i(\boldsymbol{\xi}^{\nu}) = \underbrace{\frac{N-1}{N}\xi_i^{\nu}}_{\text{signal}} + \underbrace{\frac{1}{N}\sum_{\mu\neq\nu}\xi_i^{\mu}\sum_{j\neq i}\xi_j^{\mu}\xi_j^{\nu}}_{\text{crosstalk }\eta_i^{\nu}}.
> $$
> Define the margin $m_i^{\nu}=\xi_i^{\nu}h_i(\boldsymbol{\xi}^{\nu})=\frac{N-1}{N}+\xi_i^{\nu}\eta_i^{\nu}$. If $m_i^{\nu}>0$ for all $i$, then $\boldsymbol{\xi}^{\nu}$ is stable under asynchronous sign updates. For the full derivation, see [Why Classical Hopfield Networks Converge and Why Hebbian Learning Works](CHEME-5820-L6a-Advanced-Classical-HopfieldConvergence-HebbianLearning-Proof-Spring-2026.ipynb).

Taken together, the convergence theorem answers "will asynchronous dynamics stop?" and the retrieval stability theorem answers "when is a specific stored pattern locally stable?" Neither result alone guarantees that a noisy cue will converge to the intended stored memory.
...
- **Antipatterns**: The network can converge to the bitwise inverse of a stored memory (for example, if $\boldsymbol{\xi}^{\mu}$ is stored, then $-\boldsymbol{\xi}^{\mu}$ can also be stable). This occurs because the Hebbian rule creates a symmetric energy landscape that can support both a pattern and its inverse.
____
